...
int main(argc,argv) int argc; char *argv[]; {
    ...
    if (rank == 0){ //* (1) - Check if the rank of current process is 0

        dest = 1; //* (2) - if it does, it sets rank of receiver (dest) to 1
        source = 1; 
        MPI_Send(&outmsg, 1, MPI_CHAR, dest, tag, MPI_COMM_WORLD); //* (3)- Then, it send message to Rank 1
        MPI_Recv(&inmsg, 1, MPI_CHAR, source, tag, MPI_COMM_WORLD, &Stat);

    } else if (rank == 1) { //* (4) - if the rank is not 0, it sets rank of receiver (dest) to 0

        dest = 0; 
        source = 0; 
        MPI_Recv(&inmsg, 1, MPI_CHAR, source, tag, MPI_COMM_WORLD, &Stat); //* (5) - Then, it receives message from Rank 0
        MPI_Send(&outmsg, 1, MPI_CHAR, dest, tag, MPI_COMM_WORLD); 

    }

    MPI_Get_count(&Stat, MPI_CHAR, &count); //* (6) - Get the count of the message received for printing the message
